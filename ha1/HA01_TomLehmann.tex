...
%    % The main labels of the axes can be places using `node`s:
	 ...
	 \draw [color = blue] (-2.2,1) -- (2.2,1) node [right] {$\frac {\pi}2$};
	 \draw [color = blue] (-2.2,-1) -- (2.2,-1) node [right] {$- \frac {\pi}2$};
	 \draw [color = blue] (2,-1.5) -- (2,1.5) node [right] {$\pi$};
     \draw [color = blue] (0,-1.5) -- (0,1.5) node [right] {$0$};
	 \draw [color = blue] (-2,-1.5) -- (-2,1.5) node [right] {$- \pi$};
	\end{scope}
\end{tikzpicture}
\end{floatingfigure}
...
Beschränkt? Nein, da die Gerade $x=0 \in B$.\\
...
% Tooom, warst du betrunken? ;P
Beschränkt? Nein, da der Körper unbegrenzt groß ist.
Kompakt? Nein, da $C$ zwar abgeschlossen aber nicht beschränkt.\\\\

% Was sind topologische Eigenschaften? :o
TODO: A-C
\newpage
...
\textsc{Max Bachl} \textbf{WARUM ist das eingerückt?}
...
%    % The main labels of the axes can be places using `node`s:
	 ...
	 \draw [-] (1,1) -- (2,1);
	 \fill[pattern color=blue!60, pattern=north west lines] (1,1) rectangle (2,2);
	\end{scope}
\end{tikzpicture}
\end{floatingfigure}
...
Von der Logik her gilt das auf jeden Fall. Bleibt zu beweisen warum... % Max
...
\subsubsection*{\textbf{(ii)}}
Der cos bei $b_k$ schwankt ja immer zwischen ein paar Werten. Wir nehmen die Folgen an gleichen Werten als Teilfolgen, haben dann konstante Folgen, die soundso konvergieren --- Max
